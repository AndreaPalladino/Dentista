...
<div class="container  mt-3 py-5">
    ...
    <div class="row">
        <div class="col-12">
            ...
            <form method="POST" action="{{route('contacts.update', compact('contact'))}}" class="py-5">
                @method('PUT')
                @csrf
                <div class="form-group mt-4">
                  <label class="text-center text-white mb-2"for="exampleInputEmail1">Nome e Cognome</label>
                  <input value="{{$contact->name}}" name="name" type="text" class="w-50 d-block mx-auto box-border" id="inputName" aria-describedby="emailHelp">
                </div>
                <div class="form-group mt-4">
                  <label class="text-center text-white mb-2"for="exampleInputPassword1">Email</label>
                  <input value="{{$contact->email}}" name="email" type="email" class="w-50 d-block mx-auto box-border" id="inputMail">
                </div>
                <div class="form-group mt-4">
                    <label class="text-center text-white mb-2"for="exampleInputPassword1">Telefono</label>
                    <input value="{{$contact->phone}}" name="phone" type="phone" class="w-50 d-block mx-auto box-border" id="inputPhone" >
                  </div>
